...
| `minimax-music-playlist` | Generate personalized playlists by scanning local music libraries (Apple Music, QQ Music, Spotify, NetEase). Builds a taste profile (genre, mood, language, vocal preferences), plans a themed tracklist, generates songs concurrently, creates an album cover, and refines the profile from feedback. | Official |
